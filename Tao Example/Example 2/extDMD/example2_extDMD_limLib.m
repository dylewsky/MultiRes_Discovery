...
xRaw = x;
% x = poolData(x.',nVars,polyorder,usesine);
% x = x.';
...
for q = 1:size(res_list,1)
    ...
    xL = reshape(xSample, nFeat, sampleSteps/nSplit, nSplit);
...
%         b = mr_res{q,k}.b;
    ...
    all_om_sq_thresh = all_om_sq(floor(om_thresh_pct*length(all_om_sq)));
    all_om_sq = all_om_sq(1:floor(om_thresh_pct*length(all_om_sq)));   
...
%         end
        ...
        mr_res{q,k}.om_class = om_class;
        mr_res{q,k}.om_post = om_post;
...
save('mr_res_2_ext_limLib.mat', 'mr_res');
save('res_list_2_ext_limLib.mat', 'res_list');
save('gmm_list_2_ext_limLib.mat', 'gmmList', 'nComponents');
...
if exist('mr_res','var') == 0
    ...
    load('gmm_list_2_ext_limLib.mat');
end
...
dupShift = 0.00; %multiplier to shift duplicate values so they can be visually distinguished
...
%         xr_window = zeros(r,steps_per_window);
%         for m = 1:r
...
%         scrollsubplot(plotDims(1),plotDims(2),plotDims(2)*q-1:plotDims(2)*q);
    ...
    if export_result == 1
        if q == 1
            export_fig 'manyres_opt_2_ext_limLib' '-pdf';
%             print(gcf, '-dpdf', 'manyres_opt.pdf'); 
        else
            export_fig 'manyres_opt_2_ext_limLib' '-pdf' '-append';
