...
load('Three_Body_Data_Slow_Downsample.mat');
x = pos;
TimeSpan = tspan;

r = 4; %rank to fit w/ optdmd
...
if use_median_freqs == 1
    load('km_centroids_slow_recursion.mat');
...
save('mwDMD_params_slow_recursion.mat','r','nComponents','initialize_artificially','use_last_freq','use_median_freqs','wSteps','nSplit','nSteps','nVars','thresh_pct','stepSize','nSlide');
...
if exist('mr_res','var') == 0
    try
        load('mwDMD_mr_res_slow_recursion_i2.mat');
    catch ME
...
if use_median_freqs == 0
    save('km_centroids_slow_recursion.mat','km_centroids');
end
...
if use_median_freqs == 0
    save('mwDMD_mr_res_slow_recursion.mat', 'mr_res', '-v7.3');
else
    save('mwDMD_mr_res_slow_recursion_i2.mat', 'mr_res', '-v7.3');
end
...
if use_median_freqs == 0
    save('mwDMD_allModes_slow_recursion.mat','allModes','allFreqs');
else
    save('mwDMD_allModes_slow_recursion_i2.mat','allModes','allFreqs');    
end
...
% 
% if use_median_freqs == 0
%     save('modeSeries_slow_recursion.mat','modeStack','t_step');
% else
%     save('modeSeries_slow_recursion_i2.mat','modeStack','t_step');
% end
...
save('recon_gaussian_filter_slow_recursion.mat','recon_filter','recon_filter_sd');
for k = 1:nSlide
    w = mr_res{k}.w;
    b = mr_res{k}.b;
    all_b(:,k) = b;
...
save('mwDMD_sep_recon_slow_recursion.mat','xr_sep','tspan','nComponents','suppress_growth');
