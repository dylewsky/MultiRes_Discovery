...
wSteps = 4*5500;
nSplit = floor(size(x,2)/wSteps); %number of windows if they were non-overlapping
...
save('discrete_window_DMD_params.mat','r','nComponents','initialize_artificially','use_last_freq','use_median_freqs','wSteps','nSplit','nSteps','nVars','thresh_pct','stepSize','nSlide');
...
if exist('mr_res','var') == 0
    if nonlinear == 0
        try
            load('discrete_window_DMD_mr_res_i2_linear.mat');
        catch ME
            load('discrete_window_DMD_mr_res_linear.mat');
        end
    else
        try
            load('discrete_window_DMD_mr_res_i2.mat');
        catch ME
            load('discrete_window_DMD_mr_res.mat');
        end
    end
end
...
if nonlinear == 0
    if use_median_freqs == 0
        save('discrete_window_DMD_mr_res_linear.mat', 'mr_res');
    else
        save('discrete_window_DMD_mr_res_i2_linear.mat', 'mr_res');
    end
else
    if use_median_freqs == 0
        save('discrete_window_DMD_mr_res.mat', 'mr_res');
    else
        save('discrete_window_DMD_mr_res_i2.mat', 'mr_res');
    end
end
...
if use_median_freqs == 0
    save('discrete_window_DMD_allModes.mat','allModes','allFreqs');
else
    save('discrete_window_DMD_allModes_i2.mat','allModes','allFreqs');    
end
...
if use_median_freqs == 0
    save('discrete_window_modeSeries.mat','modeStack','t_step');
else
    save('discrete_window_modeSeries_i2.mat','modeStack','t_step');
end
...
save('discrete_window_DMD_sep_recon.mat','xr_L','xr_H','tspan');
